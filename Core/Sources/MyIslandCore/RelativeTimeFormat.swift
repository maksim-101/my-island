...
/// timer-fire jitter around a one-shot 1h/15m/at-start threshold bump (a bump fired a fraction of
/// a second late must not read "14m" at the exact 15-minute mark). Since
/// `ThresholdScheduler.thresholds` are all <= 3600s (1h) and the minutes branch is inclusive of
/// exactly 3600s, `.nearest` never touches the hours branch and needs no reconciliation with it —
/// the hours branch always floors. Every other, continuously-ticking consumer passes `.floor`.
